...
local ScoreModel    = cudacheck(model.TransEModel:double())
...
for i = 1, endIdx do
	...
	hitFil = hitFil + (subRankFil <= topEnt and 1 or 0) + (objRankFil <= topEnt and 1 or 0)
	hitRaw = hitRaw + (subRankRaw <= topEnt and 1 or 0) + (objRankRaw <= topEnt and 1 or 0)
	
	hitRel = hitRel + (relRankFil <= topRel and 1 or 0)
	rankSumRel = rankSumRel + relRankFil

	if o_o_flag then
		o_o_h = o_o_h + (subRankFil <= topEnt and 1 or 0)
		o_o_t = o_o_t + (objRankFil <= topEnt and 1 or 0)
		o_o = o_o + 1
	else
		if n_o_flag then
			n_o_h = n_o_h + (subRankFil <= topEnt and 1 or 0)
			n_o_t = n_o_t + (objRankFil <= topEnt and 1 or 0)
			n_o = n_o + 1
		elseif o_n_flag then
			o_n_h = o_n_h + (subRankFil <= topEnt and 1 or 0)
			o_n_t = o_n_t + (objRankFil <= topEnt and 1 or 0)
			o_n = o_n + 1
		else
			n_n_h = n_n_h + (subRankFil <= topEnt and 1 or 0)
			n_n_t = n_n_t + (objRankFil <= topEnt and 1 or 0)
			n_n = n_n + 1
		end	
	end
